#  Version 1.0.0
# Online code uses csv file for table schema etc but data generated is specific to data type not column name. all string columns will have randon str values
# python snowflake_python_generator.py testdf1.csv
...
def load_db_file(filename, quoted_names):
   ...
   fh = open(filename, 'rU')
    ...
    with open(filename) as fh:
        next(
            fh
        )  # next() method returns the current row and moves to the next row, first time it returns the header
        line_number += 1
        for line_list in csv.reader(fh, delimiter=",", quotechar='"'):
            ...
            if datatype.upper() == "DATE":
                ...
                fb = (
                    "(date_part(epoch_second, current_date) + (uniform(1, "
                    + str(cardinality)
                    + ", random("
                    + str(seed_1)
                    + "))))"
                ...
                fe = (
                    "::" + datatype.lower() + "(" + str(datatype_length) + ") as " + col
                )
                ...
                datatype.upper() == "INTEGER"
                or datatype.upper() == "DOUBLE"
                or datatype.upper() == "FLOAT"
            ):
            ...
            if fb == "":
                if schema != "" or datatype != "":
                    print(
                        "WARNING: Line: {0} Unknown Datatype: {1}".format(
                            line_number, datatype
                        )
                    )
            else:
                if int(cardinality) <= 0:
                    ...
                    formula = (
                        "(case when uniform(1,1000,random("
                        + str(seed_1 + 10000)
                        + "))<="
                        + str(int(null_ratio))
                        + " then null else "
                ...
                column_info = {
                    "NAME": col,
                    "DATA_TYPE": datatype,
                    "LENGTH": datatype_length,
                    "CARDINALITY": cardinality,
                    "TBL_CARDINALITY": tbl_cardinality,
                    "FORMULA": formula,
                }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Snowflake Data Generation Utility.",
        epilog="Example: snowflake_datagen.py source_file.csv",
    )
    parser.add_argument(
        "filename", action="store", help="Customer Sample file template"
    )
    parser.add_argument(
        "-sqlfile",
        "--sqlfile",
